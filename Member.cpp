...
Member::Member(string userId, string username, string password, string contact, string address , vector<string> skills, vector<DateTime> availability, float requiredHostRating, vector<Member*> blockedMembers, float averageHostRating, float averageSupporterRating, string city, int creditPoints):User(move(userId), move(username), move(password), move(contact), move(address)){
    this->skills = move(skills);
    this->availability = move(availability);
...
vector<DateTime> Member::getAvailability(){
...
void Member::setAvailability(vector<DateTime> availability){
    this->availability = move(availability);
}
...
//Methods
void Member::bookService(){

}
void Member::listAvailability(vector<Member*> memberList){
    if (memberList.empty()) {
        cout << "No supporters are available." << endl;
        return;
    }
    cout << "Supporters available:" << endl;
    for (int i = 0; i < memberList.size(); i++) {
        cout << memberList[i]->getUsername() << endl;
        cout << "Skills: " << endl;
        for (int j = 0; j < memberList[i]->getSkills().size(); j++) {
            cout << memberList[i]->getSkills()[j] << endl;
        }
        cout << "Availability: " << endl;
        for (int j = 0; j < memberList[i]->getAvailability().size(); j++) {
            memberList[i]->getAvailability()[j].Display();
        }
    }
}
...
void Member::updateAvailability(){
    cout << "Enter the number of availabilities you want to add: ";
    int num;
    cin >> num;
    for (int i = 0; i < num; i++) {
        cout << "Availability " << i + 1 << endl;
        cout << "Enter the year: ";
        int year;
        cin >> year;
        ...
        availability.push_back(dateTime);
    }
}
